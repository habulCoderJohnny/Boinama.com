                ...
                <div class="col-lg-8">
                    <div class="user-profile-details">
                        <div class="order-history">
                            ...
                            <form id="subscribe-form" class="pay-form" class="form-horizontal" action="" method="POST"
                                ...
                                <div class="form-group">
                                    <label class="control-label col-sm-4" for="name">{{ __('Current Balance') }}
                                        {{ App\Models\Product::vendorConvertPrice(Auth::user()->balance) }}</label>
                                </div>

                                <div class="row">
                                    <div class="col-lg-12">
                                        <label class="control-label col-sm-12"
                                            for="name">{{ __('Select Payment Method') }} * </label>
                                    </div>
                                    <div class="col-lg-12">
                                        <select class="form-control" name="method" id="option" class="option"
                                            required="">
                                            <option value="" selected disabled>{{ __('Select Payment Method') }}
                                            </option>
                                            @foreach ($gateways as $gateway)
                                                <option value="{{ $gateway->keyword }}" data-val="{{ $gateway->keyword }}"
                                                    data-form="{{ $gateway->showDepositLink() }}"
                                                    data-href="{{ route('front.load.payment', [$gateway->keyword, 0]) }}">
                                                    {{ $gateway->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="form-group mt-2">
                                    <div class="row">
                                        <div class="col-md-12">
                                            <label class="control-label col-sm-12" for="name">{{ __('Amount') }} *
                                            </label>
                                            <div class="input-group mb-3">
                                                <input type="text" id="depositAmount" name="amount" class="form-control"
                                                    placeholder="{{ __('Deposit Amount') }}" value="{{ old('amount') }}"
                                ...
                                <div class="add-product-footer">
                                    <button name="addProduct_btn" id="final-btn" type="submit"
                                        class="mybtn1">{{ __('Deposit') }} </button>
                                </div>
                            </form>
                        </div>
                    </div>

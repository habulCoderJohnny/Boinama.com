                        ...
                        <form class="uplogo-form" id="geniusform" action="{{ route('admin-gs-update') }}" method="POST"
                            ...
                            <div class="loader-switcher">
                                ...
                                <div class="action-list">
                                    <select
                                        class="process select droplinks {{ $gs->is_loader == 1 ? 'drop-success' : 'drop-danger' }}">
                                        <option data-val="1" value="{{ route('admin-gs-isloader', 1) }}"
                                            {{ $gs->is_loader == 1 ? 'selected' : '' }}>{{ __('Activated') }}</option>
                                        <option data-val="0" value="{{ route('admin-gs-isloader', 0) }}"
                                            {{ $gs->is_loader == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}</option>
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="special-box">
                        ...
                        <form class="uplogo-form" id="geniusform" action="{{ route('admin-gs-update') }}" method="POST"
                            ...
                            <div class="loader-switcher">
                                ...
                                <div class="action-list">
                                    <select
                                        class="process select droplinks {{ $gs->is_admin_loader == 1 ? 'drop-success' : 'drop-danger' }}">
                                        <option data-val="1" value="{{ route('admin-gs-is-admin-loader', 1) }}"
                                            {{ $gs->is_admin_loader == 1 ? 'selected' : '' }}>{{ __('Activated') }}
                                        </option>
                                        <option data-val="0" value="{{ route('admin-gs-is-admin-loader', 0) }}"
                                            {{ $gs->is_admin_loader == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}
                                        </option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="currrent-logo">
                                ...
                                <img src="{{ $gs->admin_loader ? asset('public/assets/images/' . $gs->admin_loader) : asset('public/assets/images/noimage.png') }}"
                                    alt="">
                            </div>

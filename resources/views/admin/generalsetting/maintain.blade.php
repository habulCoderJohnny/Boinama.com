                                ...
                                <div class="row justify-content-center">
                                    ...
                                    <div class="col-lg-6">
                                        <div class="action-list">
                                            <select
                                                class="process select droplinks {{ $gs->is_maintain == 1 ? 'drop-success' : 'drop-danger' }}">
                                                <option data-val="1" value="{{ route('admin-gs-maintain', 1) }}"
                                                    {{ $gs->is_maintain == 1 ? 'selected' : '' }}>{{ __('Activated') }}
                                                </option>
                                                <option data-val="0" value="{{ route('admin-gs-maintain', 0) }}"
                                                    {{ $gs->is_maintain == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}
                                                </option>
                                            </select>
                                        </div>
                                    </div>

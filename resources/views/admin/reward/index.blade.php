                    ...
                    <div class="heading-area">
                        ...
                        <form id="geniusform" action="{{ route('admin-reward-info-update') }}" method="POST">
                            ...
                            <div class="action-list mr-2">
                                <select
                                    class="process select droplinks {{ $gs->is_reward == 1 ? 'drop-success' : 'drop-danger' }}">
                                    <option data-val="1" value="{{ route('admin-gs-is_reward', 1) }}"
                                        {{ $gs->is_reward == 1 ? 'selected' : '' }}>{{ __('Activated') }}</option>
                                    <option data-val="0" value="{{ route('admin-gs-is_reward', 0) }}"
                                        {{ $gs->is_reward == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}</option>
                                </select>
                            </div>
                            ...
                            <div class="action-list ml-2 mr-2 d-inline-block">
                                <input type="number" min="1" name="reward_point" class="form-control"
                                    value="{{ $gs->reward_point }}" placeholder="{{ __('Reward point') }}">
                            </div>
                            {{ __('To (USD) Dolar ($)') }}
                            <div class="action-list ml-2">
                                <input type="number" min="0" name="reward_dolar" class="form-control"
                                    value="{{ $gs->reward_dolar }}" placeholder="{{ __('USD') }}">
                            </div>
                            <div class="action-list ml-2">
                        ...
                        <div class="body-area">
                            ...
                            <form id="geniusform" action="{{ route('admin-reward-update') }}" method="POST">
                                ...
                                <div class="featured-keyword-area">
                                            ...
                                            <div class="feature-area">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-lg-6">
                                                        ...
                                                        <input type="number" name="order_amount[]" class="input-field"
                                                            placeholder="{{ __('Order Amount (USD)') }}" min="0"
                                                            value="{{ $data->order_amount }}">
                                                    </div>
                                                    <div class="col-lg-6">
                                                        <input type="number" name="reward[]" class="input-field"
                                                            placeholder="{{ __('Reward') }}" min="0"
                                                            value="{{ $data->reward }}" />
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <a href="javascript:;" id="whole-btn" class="add-fild-btn"><i class="icofont-plus"></i>
                                        {{ __('Add More Field') }}</a>
                                </div>
                                ...
                                <div class="row text-center">
                                    ...
                                    <div class="col-lg-12">
                                        <button class="addProductSubmit-btn"
                                            type="submit">{{ __('Update Reward') }}</button>
                                    </div>
                                </div>
                            </form>
                        </div>

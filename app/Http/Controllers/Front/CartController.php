...
class CartController extends Controller
{
    ...
    public function addbyone(Request $request)
    {
        ...
        $data[4] = $cart->items[$itemid]['cart_id'];
        $data[5] = $discount;
        $data[7] = $discount;
        $data[8] = $cart->items[$itemid]['discount_percentage'];
        $data[6] = $cart->items[$itemid]['discount_percentage'];
        $data[0] = round($data[0] + $data['discount1'] * $curr->value, 2);
